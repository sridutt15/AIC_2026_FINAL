...
export default function EvidencePanel({ finding, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-20 w-[26rem] overflow-y-auto border-l border-gray-200 bg-white p-6 shadow-lg">
      ...
      <dl className="mt-5 space-y-4 text-sm">
        <div>
          ...
          <dd className="mt-1 text-gray-800">{evidence.method}</dd>
        </div>
          ...
          {showLineage && (
            <ol className="mt-2 space-y-1.5 border-l-2 border-indigo-200 pl-4">
              {evidence.lineage.map((step, i) => (
                <li key={i} className="relative text-xs text-gray-700">
